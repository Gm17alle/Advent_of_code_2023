...
class Hand implements Comparable<Hand> {
    ...
    public int getType(char[] hand) {
        ...
        if(cards.containsValue(5) && cards.keySet().contains('J')) {
            return 6;
        }
        
        var countJ = cards.getOrDefault('J', 0);
        cards.remove('J');
        for(char c: cards.keySet()) {
            if(cards.get(c) > cards.get(mostFreq)) {
                mostFreq = c;
            }
        }
        
        cards.put(mostFreq, cards.get(mostFreq) + countJ);
